...
func _physics_process(delta: float) -> void:
	...
	var collision_result = body.move_and_collide(motion)
	if collision_result:
		_on_hit_obstacle(collision_result)
	else:
		body.global_position = body.global_position.snapped(Vector2(1, 1)) # Pixel perfect
		traveled = (body.global_position - origin_position).length()
		var half = patrol_distance * 0.5
		var offset = (body.global_position - origin_position).dot(travel_direction)
		if offset > half:
			_flip_direction()
		elif offset < -half:
			_flip_direction()
...
func _on_hit_obstacle(collision_result) -> void:
	...
	if n.dot(travel_direction) < -0.1:
		_flip_direction()
	body.global_position = body.global_position.snapped(Vector2(1, 1)) # Pixel perfect
	body.global_position += n * collision_result.get_remainder().length()
