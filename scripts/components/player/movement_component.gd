...
func _ready() -> void:
	...
	if animation_manager:
		animation_manager.fade_in_ended.connect(_set_movement_disabled)
		animation_manager.fade_out_ended.connect(_set_movement_enabled)

func _on_stamina_changed(current: float, max_val: float) -> void:
	stamina_changed.emit(current, max_val)

func _set_movement_disabled() -> void:
	movement_enabled = false
	body.velocity.x = 0
	is_idle.emit()

func _set_movement_enabled() -> void:
